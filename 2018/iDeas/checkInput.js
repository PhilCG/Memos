// https://github.com/xuxuepu/xxp-verification
export default {
  /* JS验证
  用途：校验ip地址的格式
  输入：strIP：ip地址
 ...
 */
  ...
  getTotalBytes: function (varField) {
    ...
    for (i = 0; i < varField.value.length; i++) {
      if (varField.value.charCodeAt(i) > 127) { totalCount += 2 } else { totalCount++ }
    }
    return totalCount
  },
  ...
  getFirstSelectedValue: function (checkboxID) {
    ...
    if (document.all(checkboxID).length > 0) {
      ...
      if (document.all(checkboxID).checked) { value = document.all(checkboxID).value }
    }
    return value
  },
  ...
  getFirstSelectedIndex: function (checkboxID) {
    ...
    if (document.all(checkboxID).length > 0) {
      ...
      if (document.all(checkboxID).checked) { value = -1 }
    }
    return value
  },
